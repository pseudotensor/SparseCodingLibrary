...
def pareto():
    ...
    for iter in range(0, iterations):
        ...
        # Warmup
        scl.sparse_code(X, p.dict_size, p.target_sparsity, p.iterations, metric='pyksvd',
                        dict_updater=p.dict_updater)
        for updater in sparse_updaters:
            tmp = time.time()
            result = scl.sparse_code(X, p.dict_size, p.target_sparsity, p.iterations, metric='pyksvd',
        ...
        times.append(time.time() - tmp)
        accuracy.append(scl.rmse(X, ksvd_result[0].transpose(), ksvd_result[1].transpose()))
